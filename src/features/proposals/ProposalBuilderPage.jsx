...
export function ProposalBuilderPage({ data, reload, setScreen, initialClient }) {
  ...
  const proposalId = useMemo(() => nextProposalId(data.proposals, data.settings), [data.proposals, data.settings]);
  ...
  const handleSaveDraft = async () => {
    ...
    setSaved(true);
    setTimeout(() => { setSaved(false); setScreen("proposals"); }, 1200);
  };
      ...
      {step === 4 && (
        <div>
          <div className="send-actions">
            ...
            <button className="button" onClick={handleSaveDraft}>
...
function SendEmailModal({ client, proposalId, lineItems, totals, paymentLink, frequency, data, onSent, onClose }) {
  ...
  const defaultSubject = (settings.email?.subjectTemplate || "Tripclap Proposal {{id}} for {{agency}}")
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card send-email-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="gmail-how-it-works">
          <div className="gmail-step"><span>1</span> Click the button below — Gmail compose opens in a new tab</div>
        ...
        {status === "error" && (
          <div className="form-error">
            Clipboard access was denied. Allow clipboard permission and try again, or copy the HTML manually.
          </div>
        )}

        <div className="modal-actions">
          <button className="button primary" onClick={openInGmail} disabled={!toEmail}>
            <Mail size={14} /> Open Gmail & Paste
          </button>
